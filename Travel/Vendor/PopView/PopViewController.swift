...
class PopViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func setupViews(_ popViewtype:PopViewType) {
        ...
        let img = UIImage(named: imgStr)?.resizableImage(withCapInsets: UIEdgeInsetsMake(25, 30, 25, 30))
        let imgView = UIImageView(image: img)
        imgView.frame.size = CGSize(width: 130, height: 110)
        view.addSubview(imgView)
        myTableView.frame = CGRect(x: 10, y: 12, width: 110, height: 88)
