...
- (void)viewDidLoad {
    ...
    if (self.conversation.type == EMConversationTypeGroupChat) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"people"] style:UIBarButtonItemStylePlain target:self action:@selector(pushToGroup)];
        ...
        UIView * hView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, UIScreen.mainScreen.bounds.size.width, 46)];
        hView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:hView];
        [self.view bringSubviewToFront:hView];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(10, 0, UIScreen.mainScreen.bounds.size.width - 100, 46);
        [button setContentMode:UIViewContentModeLeft];
        [button setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
        [button setImage:[UIImage imageNamed:@"date"] forState:UIControlStateNormal];
        [button setTitle:@"活动" forState:UIControlStateNormal];
        [button setTitleColor:[UIColor colorWithRed:41/255.f green:161/255.f blue:247/255.f alpha:1] forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:15];
        [button addTarget:self action:@selector(pushToActions) forControlEvents:UIControlEventTouchUpInside];
...
- (void) pushToActions {
    [NSNotificationCenter.defaultCenter postNotificationName:@"JCircleListViewController" object:@"b"];
}

- (void) pushToGroup {
    [NSNotificationCenter.defaultCenter postNotificationName:@"JCircleListViewController" object:@"a"];
}
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"JUploadMassageManager" object:@1 userInfo:@{@"id": self.serverId, @"content" : text, @"isToUser": @(true)}];
//    } else {
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"JUploadMassageManager" object:@1 userInfo:@{@"id": self.serverId, @"content" : text, @"isToUser": @(false)}];
//    }
}
